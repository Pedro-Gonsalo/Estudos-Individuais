...
procedure TForm4.lbxHabilidadesDragOver(Sender, Source: TObject; X, Y: Integer;
  State: TDragState; var Accept: Boolean);
begin
  Accept := Source <> Sender; // Só aceita o drop se a origem for diferente de quem está arrastando
end;

procedure TForm4.lbxTecnologiasDragDrop(Sender, Source: TObject; X, Y: Integer);
begin
  BtnToTecnologiasClick(Sender); // Sender = Quem invocou este método
end;
...
- Bevel: Exibe um quadro, painel ou caixa sobre um formulário ou componente-imagem com efeito
tridimensional.
